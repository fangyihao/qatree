...
class ParallelTransformerLayer(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        output_layer_init_method=scaled_init_method(init_method_std, config.num_hidden_layers)
...
class GPT2ParallelTransformer(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        hidden_size = config.hidden_size
        num_attention_heads = config.num_attention_heads
        max_sequence_length = config.seq_len
        ...
        if relative_encoding:
            ...
            self.position_embeddings = PositionalEmbeddings(hidden_size)
            # Per attention head and per partition values.
            world_size = get_model_parallel_world_size()
            self.hidden_size_per_attention_head = divide(hidden_size,
                                                         num_attention_heads)
        ...
        def get_layer():
            if use_decoder_layer:
                ...
                return ParallelTransformerLayer(config)
