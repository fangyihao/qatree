...
from transformers import (OpenAIGPTTokenizer, BertTokenizer, XLNetTokenizer, RobertaTokenizer, MobileBertTokenizer)
try:
...
from neural_tree.utils import conceptnet
...
class DataLoader(object):
    ...
    def load_resources(self, kg):
        # Load the tokenizer
        try:
            tokenizer_class = {'bert': BertTokenizer, 'xlnet': XLNetTokenizer, 'roberta': RobertaTokenizer, 'albert': AlbertTokenizer, 'mobilebert': MobileBertTokenizer}.get(self.model_type)
        except:
            tokenizer_class = {'bert': BertTokenizer, 'xlnet': XLNetTokenizer, 'roberta': RobertaTokenizer, 'mobilebert': MobileBertTokenizer}.get(self.model_type)
    ...
    def load_input_tensors(self, input_jsonl_path, max_seq_length):
        ...
        if use_cache:
            ...
            elif self.model_type in ('bert', 'xlnet', 'roberta', 'albert', 'mobilebert'):
