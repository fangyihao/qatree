...
class Tokenizer(object):
    ...
    def __call__(self, text, process_fn=None, **kwargs):
...
class GLMTokenizer(GPT2BPETokenizer):
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path: Union[str, os.PathLike], *init_inputs, **kwargs):
        ...
        kwargs = {"add_block_symbols": True, "cache_dir": None, 
                    "add_sentinel_token": 0, "add_task_mask": None,
                    "add_decoder_mask": False}
        
        model_type = "roberta"
        if model_type is None:
            model_type = 'gpt2'
        tokenizer = GLMTokenizer(model_type, **kwargs)
    ...
    def __call__(self, text, text_pair=None, padding="max_length", truncation=True, max_length=512, return_token_type_ids=True, return_special_tokens_mask=True):
        """run preprocessing and encode text as Ids"""
        text = ' '.join(text.split(' ')[:int(max_length*0.8) - (0 if text_pair is None else len(text_pair.split(' ')))])
        
        source_text = text if text_pair is None else text + text_pair
        
        mask_token = 'MASK'
        mask_id = self.get_command(mask_token).Id
        sop_id = self.get_command('sop').Id
        eop_id = self.get_command('eop').Id
        pad_id = self.get_command('pad').Id
        ...
        source_tokens = self.EncodeAsIds(source_text).tokenization
        
        attention_mask = [1] * len(source_tokens)
        source_tokens = pad_to(source_tokens, max_length, pad_id)
        attention_mask += [0] * (len(source_tokens) - len(attention_mask))
        position_ids = list(range(len(source_tokens)))
        block_position_ids = [0] * len(source_tokens)
        
        
        token_type_ids = [0] * len(source_tokens)
        special_tokens_mask = [1] * len(source_tokens)
        ...
        sample = {'input_ids': np.array(tokens, dtype=np.int64), 'attention_mask': np.array(attention_mask, dtype=np.int64),
                  'position_id': np.array(position_ids, dtype=np.int64), 'special_tokens_mask':special_tokens_mask, 'token_type_ids':token_type_ids}
        return sample
