...
def add_data_arguments(parser):
    ...
    parser.add_argument('-ih', '--inhouse', type=util.bool_flag, nargs='?', const=True, help='run in-house setting')
...
def add_additional_arguments(parser):
    ...
    parser.add_argument('--cuda', default=True, type=util.bool_flag, nargs='?', const=True, help='use GPU')
    parser.add_argument('--seed', default=0, type=int, help='random seed')
    parser.add_argument('--debug', default=False, type=util.bool_flag, nargs='?', const=True, help='run in debug mode')
    args, _ = parser.parse_known_args()
    if args.debug:
